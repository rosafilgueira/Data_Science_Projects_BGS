...
class FindLocation(GenericPE):
    def __init__(self):
        ...
        self._add_output('output_auroras_no_uk')
        self._add_output('output_others')
        self.UK_countries={}
        uk_filename= "uk-regions.json"
	...
	if data['label'] == 'Aurora':
		...
		country = 'GB'
                ...
                self.write('output_auroras_no_uk', [data])
	else:
    ...
    def find_country(self, tweet):
	    ...
	    if country_name:
                country_idx = self.UK_countries['name'].index(country_name[0])
                return self.UK_countries['region'][country_idx]
	    ...
	    if country_name:
                country_idx = self.UK_countries['name'].index(country_name[0])
		self.log("Region %s" %  self.UK_countries['region'][country_idx])
                return self.UK_countries['region'][country_idx]
	    	#return country_name[0]
        elif tweet['country']:
    ...
    def load_countries(self, uk_file):
        UK_countries = {'name': [], 'region':[], 'coord':[] , 'subregion':[], 'postal-code':[]}
	...
	print("filename is %s" % filename)
		...
		if 'name' in f['properties']:
                    UK_countries['name'].append(f['properties']['name'].encode('utf-8'))
		
		if 'region' in f['properties']:	
                    UK_countries['region'].append(f['properties']['region'].encode('utf-8'))
		if 'region' in f['properties']:	
                    UK_countries['subregion'].append(f['properties']['subregion'].encode('utf-8'))
		if 'postal-code' in f['properties']:	
                    UK_countries['postal-code'].append(f['properties']['postal-code'])
		coord = f[u'geometry'][u'coordinates'][0]
                if len(coord)==1: coord = coord[0]
                UK_countries['coord'].append(coord)
            except ValueError:
                 continue
...
class WriteJson(GenericPE):
    ...
    def _postprocess(self):	
	name=RESULTS_DIR+"Auto_classification_8_9_May_2016_uk.json"
        data_twitter={}
