...
class FindState(GenericPE):
    def __init__(self):
        GenericPE.__init__(self)
        self._add_input ('input')
        self._add_output('output_auroras_us')
        self._add_output('output_auroras_no_us')
        self._add_output('output_others')
        self.US_states={}
        filename= "us-states.json"
        self.US_states=self.load_states(filename)
    def _process(self, inputs):
        data = inputs['input']
	if data['label'] == 'Aurora':
            state = self.find_state(data)
            if state:
                self.write('output_auroras_us', [data, state])
            else:
                self.write('output_auroras_no_us', [data])
	else:
...
class MapAuroraState(GenericPE):
    def __init__(self):
        GenericPE.__init__(self)
	...
	if state not in self.aurora:
        ...
        #aurora_state, aurora_score= self.max_aurora
        # 
        ##if self.aurora[state] > aurora_score:
        #    aurora_score = self.aurora[state]
        #    self.max_aurora = state, aurora_score 
        #self.log("!!!US-State with more Aurora is %s, score = %s"  % (state, aurora_score))
        self.log("!!!US-State with more Aurora is %s, score = %s"  % (state, self.aurora[state]))
        self.write('output', [state, self.aurora[state]])
        #self.write('output', [state, aurora_score])

class ReduceAuroraState(GenericPE):
    def __init__(self):
        GenericPE.__init__(self)
	...
	self.total_tweets = 0
    def _process(self, inputs):
        state, aurora_score = inputs['input']
	self.total_tweets += 1
...
class ReduceAuroraNoUs(GenericPE):
    def __init__(self):
        GenericPE.__init__(self)
        self._add_input ('input', grouping='global')
        self.total_tweets = 0
	...
	self.log("--->!!!Total Aurora-tweets from Non-US is %s !!!<----" % self.total_tweets)
...
class WriteJson(GenericPE):
    ...
    def _postprocess(self):	
	name=RESULTS_DIR+"Auto_classification_8_9_May_2016_us.json"
        data_twitter={}
...
findstate=FindState()
mapaurora=MapAuroraState()
reduceaurora=ReduceAuroraState()
reduceaurora_no_us=ReduceAuroraNoUs()
...
graph.connect(classify, 'output', findstate, 'input')
graph.connect(findstate, 'output_auroras_us', mapaurora, 'input')
graph.connect(findstate, 'output_auroras_no_us', reduceaurora_no_us, 'input')
graph.connect(findstate, 'output_others', reduceothers, 'input')
graph.connect(mapaurora, 'output', reduceaurora, 'input')
